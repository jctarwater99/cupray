  ...
  queries.clearDailyRequests(() => {
    getCategoriesWithActiveCount((results) => {
      ...
      while (idList.length > 0) {
        ...
        queries.getRequestsInCategory(
          curNum,
          (results, cnt, placeholder) => {
            var tickets2 = [];
            var i;
            for (i = 0; i < results.length; i++) {
              var j;
...
function scheduleForCat(catNum, startDayNum) {
  queries.getCategories(
    (results, catNum, startDayNum) => {
      if (catNum < results.length) {
        cat = results[catNum];
        queries.getRequestsInCategory(
          cat.tagID,
          (results, cat, startDayNum) => {
                  ...
                  if (results[j].id == id) {
                    ...
                    scheduleNotificationDate(
                      ...
                      queries.insertReminder({
                        reminderID: curReminder.reminderID,
                        requestID: curReminder.requestID,
                        furthestDate: curReminder.furthestDate.toString(),
                      });
                      reminderCnt++;
                    });
